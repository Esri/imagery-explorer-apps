...
export const selectDayFromCalendar = async (page: Page, day: string) => {
    ...
    const dropdownOptions = await yearSelectionDropdown
        .getByTestId('dropdown-option-' + year)
        .all();
    const dropdownOptionForTargetYear = dropdownOptions.find(
...
export const formatInUTCTimeZone = (timestamp: number): string => {
 ...
 */
export const urlHashContains = async (
    page: Page,
    substring: string
): Promise<boolean> => {
 ...
 */
export const clickOnMap = async (
    page: Page,
    x: number,
    y: number,
    waitForSentinel2ExportImageResponse = false
) => {
    ...
    if (waitForSentinel2ExportImageResponse) {
        ...
        const exportImagePromise = page.waitForResponse(
            (request) =>
                request
                    .url()
                    .includes('Sentinel2L2A/ImageServer/exportImage') &&
                request.status() === 200,
            {
