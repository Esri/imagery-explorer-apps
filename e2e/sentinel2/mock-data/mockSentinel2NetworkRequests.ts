...
export const mockSentinel2NetworkRequests = async (page: Page) => {
    ...
    await page.route('**/Sentinel2L2A/ImageServer/query?**', async (route) => {
        ...
        if (returnExtentOnly === 'true') {
            console.log(
                'Mocked Sentinel-2 scenes query for extent only intercepted'
            );
            await route.fulfill({
                status: 200,
                contentType: 'application/json',
        ...
        if (objectIds && mockedFeatureByObjectIdMap.has(+objectIds)) {
            console.log(
                `Mocked Sentinel-2 scenes query for objectid ${objectIds} intercepted`
            );
        ...
        console.log(
            'Mocked Sentinel-2 scenes query intercepted, but no conditions matched, passing through'
        );
    ...
    await page.route(
        ...
        async (route) => {
            console.log(
                'Mocked EstimateRasterAnalysisCost submitJob request intercepted'
            );
            await route.fulfill({
                status: 200,
                contentType: 'application/json',
    ...
    await page.route(
        ...
        async (route) => {
            console.log(
                'Mocked EstimateRasterAnalysisCost job status request intercepted'
            );
            await route.fulfill({
                status: 200,
                contentType: 'application/json',
    ...
    await page.route(
        ...
        async (route) => {
            ...
            await route.fulfill({
                ...
                body: JSON.stringify({
                    encodedServiceURL:
                        'https://service.arcgis.com/mockedOrg/arcgis/rest/services/mocked_Sentinel_2_Scene_service_name/ImageServer',
                    ...
                    success: true,
                    type: 'Image Service',
                    typeKeywords: ['Dynamic Imagery Layer'],
                    isView: false,
                }),
            });
        }
...
export const resetMockSentinel2NetworkRequest = async (page: Page) => {
    ...
    await page.unroute('**/Sentinel2L2A/ImageServer/query?**');
    ...
    await page.unroute('**/sharing/rest/portals/*/isServiceNameAvailable?**');
    await page.unroute(
        '**/sharing/rest/community/users/*?f=json&returnUserLicensedItems=true**'
    );
    await page.unroute('**/sharing/rest/content/users/*/createService');
