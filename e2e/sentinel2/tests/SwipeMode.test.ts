...
test.describe('Sentinel-2 Explorer - Swipe', () => {
    ...
    test('Swipe Mode functionalities', async ({ page }) => {
        ...
        const swipeLayerSelectorLeft = page.getByTestId(
            'swipe-layer-selector-left'
        );
        const swipeLayerSelectorRight = page.getByTestId(
            'swipe-layer-selector-right'
        );
        ...
        await expect(swipeLayerSelectorLeft).toHaveAttribute(
        ...
        await expect(swipeLayerSelectorRight).toHaveAttribute(
            'data-acquisition-date',
            DATE_RIGHT
        );
