...
test.describe('Sentinel-2 Explorer - Save Panel', () => {
    ...
    const APP_URL =
        DEV_SERVER_URL + '/#mapCenter=-117.07809%2C34.03876%2C13.516';
    ...
    test('save panel with multiple Sentinel-2 scenes selected', async ({
        page,
    }) => {
        // Navigate to the app with swipe mode on and two scenes selected
        await page.goto(
            APP_URL +
                '&mode=swipe&mainScene=2023-08-01%7CNDVI+Colorized+for+Visualization%7C20498195&secondaryScene=2024-01-03%7CNDVI+Colorized+for+Visualization%7C146597'
        );
    ...
    test('save panel in index mask tool', async ({ page }) => {
        ...
        await page.goto(
            APP_URL +
                '&mode=analysis&mainScene=2023-08-01%7CAgriculture+for+Visualization%7C20498195&tool=mask'
        );
    ...
    test('save panel in change detection tool', async ({ page }) => {
        ...
        await page.goto(
            APP_URL +
                '&mode=analysis&mainScene=2023-08-01%7CShort-wave+Infrared+for+Visualization%7C20498195&secondaryScene=2024-01-03%7CNatural+Color+for+Visualization%7C146597&tool=change&change=vegetation%7Ctrue%7C-2%2C2'
        );
    ...
    test('reject credits for a job in the job list', async ({ page }) => {
        ...
        await page.goto(
            APP_URL +
                '&mode=find+a+scene&mainScene=2023-08-01%7CNDVI+Colorized+for+Visualization%7C20498195'
        );
...
const triggerSaveJob = async (
    ...
    const saveJobLaunchButton = saveJobOption.locator(
        '[data-element="button"]'
    );
 ...
 */
const testSaveAsArcGISOnlineItem = async (
    page: Page,
    {
        saveJobType,
    }: {
    ...
    await expect(openJobButton).toHaveAttribute(
        'href',
        /https:\/\/(?:[\w.-]+\.)*arcgis\.com\/home\/item.html\?id=*/i
    );
};
 ...
 */
const testPublishAsHostedImageryService = async (
    page: Page,
    {
        saveJobType,
    ...
    if (shouldRejectCredits) {
        // If we are testing the rejection of credits, click the "Reject Credits" button
        const rejectCreditsButton = jobListItem.getByTestId(
            'reject-credits-button'
        );
    ...
    const acceptCreditsButton = jobListItem.getByTestId(
        'accept-credits-button'
    );
 ...
 */
const testSaveOptionsList = async (
    page: Page,
    options: PublishAndDownloadJobType[]
) => {
    const saveOptions = page.getByTestId('save-options-list');
    await expect(saveOptions).toBeVisible();
    await expect(saveOptions).toHaveAttribute(
        'data-number-of-options',
