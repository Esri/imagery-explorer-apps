...
test.describe('Sentinel-2 Land Cover Explorer - Classification List', () => {
    ...
    test('verify the time correct parameter in exportImage request', async ({
        page,
    }) => {
        ...
        const classificationsList = page.getByTestId(
            'land-cover-classifications-list'
        );
        await expect(classificationsList).toBeVisible();

        // verify the classification list contains the expected number of items
        const classificationItems = page.locator(
            '[data-testid^="land-cover-classification-"]'
        );
        ...
        const treeClassificationItem = page.getByTestId(
            'land-cover-classification-Trees'
        );
        ...
        await expect(treeClassificationItem).toHaveAttribute(
            'data-selected',
            'true'
        );

        // verify the exportImage request contains the correct Raster Function for Trees
        const exportImageRequestPromise = page.waitForRequest((request) =>
            request
                .url()
                .includes('Sentinel2_10m_LandCover/ImageServer/exportImage')
        );
