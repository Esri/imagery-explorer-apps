...
test.describe('Sentinel-2 Land Cover Explorer - Map Popup', () => {
    ...
    test('verify the Popup displays the correct information for the selected location', async ({
        page,
    }) => {
        ...
        for (const item of popupItems) {
            const popupItem = page.getByTestId(
                `popup-item-${item.year}-${item.className}`
            );
