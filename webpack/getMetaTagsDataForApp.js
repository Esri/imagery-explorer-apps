...
const getMetaTagsDataForApp = (app) => {
    console.log(
        `Loading meta tags data for "${app || 'undefined application'}"`
    );

    if (!app) {
        console.error(
            ERROR_TERMIAL_OUTPUT_COLOR,
            'No application specified for meta tags data retrieval.'
        );
        process.exit(1);
    }
    ...
    if (!config) {
        console.error(
            ERROR_TERMIAL_OUTPUT_COLOR,
            'No configuration found. Please check src/config.json to ensure it is valid.'
        );
        process.exit(1);
    }
    ...
    if (!appConfig) {
        console.error(
            ERROR_TERMIAL_OUTPUT_COLOR,
            `No configuration found for application: "${app}". Please check src/config.json to ensure the application is defined.`
        );
        process.exit(1);
    }

    const { title, description, pathname } = appConfig;
