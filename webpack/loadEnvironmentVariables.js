...
const loadEnvironmentVariables = (envFileName) => {
    ...
    if (!fs.existsSync(envPath)) {
        console.error(
            ...
            `Environment file ${envPath} does not exist. Please create it based on .env.template and place it in the project root directory.\n`
        );
        process.exit(1);
    }
    ...
    if (Object.keys(envConfig).length === 0) {
        console.error(
            ERROR_TERMIAL_OUTPUT_COLOR,
            `No environment variables found in the environment file ${envPath}. Please check the file content.`
        );
        process.exit(1);
    }
