...
const validateEnv = (
    ...
    console.log(`Validating environment variables for "${app || 'undefined application'}"`);

    if(!app){
        console.error(ERROR_TERMIAL_OUTPUT_COLOR, 'Failed to validate environment variables: No application specified for environment variable validation.');
        process.exit(1);
    }
    ...
    if(!requiredEnvVars || requiredEnvVars.length === 0){
        console.warn(`No required environment variables defined for application: "${app}". Skipping validation.`);
        return;
    }
