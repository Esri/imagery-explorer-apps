...
const getPreloadedChangeCompareToolState = (hashParams: URLSearchParams) => {
    ...
    const defaultSelectedRange = [-255, 0]; // for DRX, we set the default selected range to be [-50, 50] since it's a reasonable range to capture most of the changes while excluding very minor changes that could be noise. Users can adjust the selected range based on their needs.

    const defaultSelectedRange2 = [0, 255]; // for DRX, we set the default second selected range to be [50, 255] to allow users to also visualize the more significant changes that are greater than 50.
    ...
    const preloadedState: ChangeCompareToolState = {
        ...
        selectedRange:
            changeCompareToolData?.selectedRange || defaultSelectedRange, // for DRX, we set the default selected range to be [-50, 50] since it's a reasonable range to capture most of the changes while excluding very minor changes that could be noise. Users can adjust the selected range based on their needs.
        selectedRange2:
