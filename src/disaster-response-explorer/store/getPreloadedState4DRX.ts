...
const getPreloadedChangeCompareToolState = (hashParams: URLSearchParams) => {
    const data = getPreloadedState4ChangeCompareTool(hashParams);
    ...
    const preloadedState: ChangeCompareToolState = {
        ...
        selectedRange: data?.selectedRange || fullPixelValuesRange, // for DRX, we set the default selected range to be [-50, 50] since it's a reasonable range to capture most of the changes while excluding very minor changes that could be noise. Users can adjust the selected range based on their needs.
    };
